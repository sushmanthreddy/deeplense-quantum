...
The CLI keeps the selected 40-epoch default. The five notebooks under `notebooks/d4_orqb/` explicitly request 50 quantum epochs. They validate and print the fresh quantum-stage `best.pt` candidate but automatically copy only the generated `validation_metrics.md` and `validation_roc_curve.png` artifacts into the dataset/run results directory as `metrics.md` and `roc_curve.png`. Checkpoints are selected for `weights/` explicitly, only after reviewing each completed run; the notebooks never promote them automatically.
