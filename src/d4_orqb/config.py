"""Configuration for the selected Model-I D4-ORQB training pipeline."""
...
class Config:
    ...
    cache_root: str = ""
    output_dir: str = ""
    stage: Literal["all", "pretrain", "quantum"] = "all"
    ...
    def validate(self) -> None:
        if not self.development_root.strip():
            raise ValueError(
                "Dataset path is empty. Set --development-root to the Model-I "
                "development directory on the training machine."
            )
        ...
        if self.stage not in ("all", "pretrain", "quantum"):
